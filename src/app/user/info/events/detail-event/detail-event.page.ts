    ...
    this.activatedRoute.paramMap.subscribe(paraMap => {
      ...
      if (this.eventId) {
        ...
        this.eventService.getEventByID(this.eventId).then(res => {
          ...
          this.meta.updateTag({ propery: 'og:title', content: this.eventLoad.name, xmlns:og='http://opengraphprotocol.org/schema/'}); 
          this.meta.updateTag({ propery: 'og:description', content: this.eventLoad.description, xmlns:og='http://opengraphprotocol.org/schema/' }); 
          this.meta.updateTag({ propery: 'og:image', content: this.eventLoad.icon, xmlns:og='http://opengraphprotocol.org/schema/'}); 
          this.meta.updateTag({ propery: 'og:url', content: `https://itesm-ca2020.web.app/user/info/events/detail-event/${this.eventLoad.id}`, xmlns:og='http://opengraphprotocol.org/schema/'}); 
          this.meta.updateTag({ propery: 'og:type', content: 'website', xmlns:og='http://opengraphprotocol.org/schema/'}); 
          this.meta.updateTag({ propery: 'fb:app_id', content: '725418228231566', xmlns:og='http://opengraphprotocol.org/schema/'}); 
